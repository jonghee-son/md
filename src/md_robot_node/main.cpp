...
md::md_robot_msg2 md_robot_message2;

ROBOT_PARAMETER_t robotParamData;
static geometry_msgs::Twist old_vel_cmd;
...
void Req_tick_timer_Callback(const ros:: TimerEvent&)
{
    req_tick++;
}
...
void InitMotorParameter(void)
{
    ...
    #if 0
                    ROS_INFO("SETTING_PARAM_STEP_PID_ROBOT_PARAM(%d)", SETTING_PARAM_STEP_PID_ROBOT_PARAM);
...
int main(int argc, char** argv)
{
    ...
    nh.getParam("md_robot_node/position_type", robotParamData.motor_position_type);
    ...
    ros::Rate r(500);       //Set the loop period -> 1ms.
    ...
    ros::Timer app_tick_timer = nh.createTimer(ros::Duration(0.200), AppTickTimerCallback);
